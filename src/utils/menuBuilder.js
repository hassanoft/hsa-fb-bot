...
export function buildIdentityBlock(ctx) {
  ...
  const pingMs = Math.max(0, Date.now() - Number(ctx.msg.timestamp));
...
/**
 * Récupère la photo de profil du compte Facebook connecté, via l'endpoint
 * public Graph API (ne nécessite pas de jeton pour une photo de profil
 * publique). Ne lève jamais d'erreur.
 */
export async function getConnectedProfilePicture(bot) {
  try {
    const url = `https://graph.facebook.com/${bot.user.id}/picture?type=large`;
...
export async function sendMenu(ctx, categoryArg) {
  ...
  // Par prudence, la photo et le texte partent toujours en deux messages
  // séparés (l'API FCA n'a pas de limite de "légende" documentée de façon
  // fiable et homogène entre les différentes libs — mieux vaut ne jamais
  // dépendre d'une limite qu'on ne peut pas vérifier).
